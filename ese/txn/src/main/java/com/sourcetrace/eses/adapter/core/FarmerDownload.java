		...
		if (ag != null && (ag.getExporter() != null || ag.getPackhouse().getExporter() != null)) {
			...
			if (!ObjectUtil.isListEmpty(farmerList)) {
				...
				farmerList.forEach(farmer -> {
					...
					/*
					 * objectMap.put("exporterCode", farmer[8] != null &&
					 * !StringUtil.isEmpty(farmer[8]) ? farmer[8].toString() :
					 * ""); objectMap.put("exporter", farmer[9] != null &&
					 * !StringUtil.isEmpty(farmer[9]) ? farmer[9].toString() :
					 * "");
					 * 
					 * 
					 * objectMap.put(TxnEnrollmentProperties.MOBILE_NUMBER,
					 * farmer[10] != null && !StringUtil.isEmpty(farmer[10]) ?
					 * farmer[10].toString() : "");
					 * objectMap.put(TxnEnrollmentProperties.NATIONALID,
					 * farmer[11] != null && !StringUtil.isEmpty(farmer[11]) ?
					 * farmer[11].toString() : "");
					 * 
					 * objectMap.put(TxnEnrollmentProperties.STATE_NAME,
					 * farmer[12] != null && !StringUtil.isEmpty(farmer[12]) ?
					 * farmer[12].toString() : "");
					 * 
					 * objectMap.put(TxnEnrollmentProperties.STATE_CODE,
					 * farmer[13] != null && !StringUtil.isEmpty(farmer[13]) ?
					 * farmer[13].toString() : "");
					 */
			...
			if (!farmerList.isEmpty()) {
				resp.put(TxnEnrollmentProperties.REVISION_NO,
						farmerList.get(0)[farmerList.get(0).length - 1].toString());
			} else {
