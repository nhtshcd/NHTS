...
public class ExporterRegistrationAction extends SwitchAction {
	...
	@Getter
	@Setter
	private String selectedExporterStatus;
	@Getter
	@Setter
	private String selectedExporterStatusFlag;
	...
	public String create() {
		...
		if (expRegis == null) {
			...
			// '0':'Applied','1':'Verified','2':'Approved'
			expRegis.setExporterStatus(0);
	...
	public String detail() {
		...
		if (id != null && !id.equals("")) {
			...
			ex = utilService.getAuditRecords("com.sourcetrace.eses.entity.ExporterRegistration", expRegis.getId());

			/*
			 * Map<String,String> relations= new HashMap<>();
			 * relations.put("village",
			 * "com.sourcetrace.eses.entity.Village##village"); ex =
			 * utilService.getAuditRecords(
			 * "com.sourcetrace.eses.entity.ExporterRegistration",expRegis.getId
			 * (),relations,"exporter_registration_aud");
			 */
	...
	public String update() throws Exception {
		...
		if (id != null && !id.equals("") && expRegis == null) {
			...
			if (expRegis.getExporterStatus() != null) {
				setSelectedExporterStatus(expRegis.getExporterStatus().toString());
				setSelectedExporterStatusFlag(expRegis.getExporterStatus().toString());
			}
			...
			if (selectedExporterStatus != null && !StringUtil.isEmpty(selectedExporterStatus)) {
				tempexpReg.setExporterStatus(Integer.valueOf(selectedExporterStatus));
			}
			...
			// if(selectedExporterStatus!=null &&
			// !StringUtil.isEmpty(selectedExporterStatus) &&
			// selectedExporterStatus.equalsIgnoreCase("2")){
			if (selectedExporterStatus != null && !StringUtil.isEmpty(selectedExporterStatus)
					&& selectedExporterStatus.equalsIgnoreCase("2")
					&& !selectedExporterStatusFlag.equalsIgnoreCase("2")) {
				String pw = StringUtil.generateStrPassword(8);
				if (tempexpReg.getTin() != null) {
					String msg = getLocaleProperty("mail_exproterRegister");
					msg = msg.replace("[pw]", pw);
					sendExpireMail(tempexpReg.getTin().trim(), tempexpReg.getName(), msg, "Exporter registered");
					StringBuilder sb = new StringBuilder();
					sb.append(getLocaleProperty("hai") + " " + tempexpReg.getName() + "\n\n\n");
					sb.append(msg);
					String message = sb.toString();
					smsService.sendSMS(smsType, tempexpReg.getMobileNo(), msg);
				}

				Role r1 = utilService.findRoleByType(Role.Type.EXPORTER_ADMIN.ordinal());
				User u = new User();
				u.setUsername(tempexpReg.getTin());
				u.setStatus(1);
				...
				u.setLoginStatus(0);
				u.setAgroChDealer(tempexpReg.getId());
				u.setRole(r1);
				u.getContInfo().setAddress1(tempexpReg.getAddr());
				u.getContInfo().setEmail(tempexpReg.getTin());
				u.getContInfo().setMobileNumbere(tempexpReg.getMobileNo());
				u.getPersInfo().setGender(tempexpReg.getPackToExitPoint());
				u.getPersInfo().setFirstName(tempexpReg.getName());
				u.setEnabled(true);
			...
			if (!ObjectUtil.isEmpty(states)) {
				for (State state : states) {
					...
					if (!StringUtil.isEmpty(name) && name != null) {
						stateArr.add(getJSONObject(String.valueOf(state.getId()),
								state.getCode() + "-" + getLanguagePref(getLoggedInUserLanguage(), state.getCode())));
					} else {
						stateArr.add(
		...
		if (!selectedState.equalsIgnoreCase("null") && (!StringUtil.isEmpty(selectedState))) {
			...
			if (!ObjectUtil.isEmpty(listLocalities)) {
				for (Locality locality : listLocalities) {
					...
					if (!StringUtil.isEmpty(name) && name != null) {
						localtiesArr.add(getJSONObject(String.valueOf(locality.getId()), locality.getCode().trim() + "-"
								+ getLanguagePref(getLoggedInUserLanguage(), locality.getCode().trim())));
					} else {
		...
		if (!selectedCity.equalsIgnoreCase("null") && (!StringUtil.isEmpty(selectedCity))) {
			...
			if (!ObjectUtil.isEmpty(listVillages)) {
				for (Village village : listVillages) {
					...
					if (!StringUtil.isEmpty(name) && name != null) {
						villageArr.add(getJSONObject(String.valueOf(village.getId()), village.getCode().trim() + "-"
								+ getLanguagePref(getLoggedInUserLanguage(), village.getCode().trim())));
					} else {
