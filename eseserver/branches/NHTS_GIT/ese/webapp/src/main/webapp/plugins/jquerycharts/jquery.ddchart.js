...
// Have fun!
	...
	/*
	 * function getCurrentBranch() { return "AAF"; }
	 */
	...
	jQuery.fn.ddBarChart = function(options) {
		...
		function buildYAxis (objSettings, strID) {
			...
			for (intValues = 0; intValues < objSettings.chartData.DATA.length; intValues++) {
				intYMax = parseInt(objSettings.chartData.DATA[intValues][objSettings.chartData.COLUMNS.indexOf('X_BAR_VALUE')]) > intYMax ? parseInt(objSettings.chartData.DATA[intValues][objSettings.chartData.COLUMNS.indexOf('X_BAR_VALUE')]) : intYMax;
			}			
			...
			for (intYLabels = (intYMin+intYStep); intYLabels <= intYMax; intYLabels += intYStep) {
					$(strID + " > .ddchart-y-axis-wrapper").append('<div class="ddchart-y-axis" style="bottom:'+((intYLabels-intYStep)/intYMax)*100+'%; height:'+(intYStep/intYMax)*100+'%; z-index:100; overflow:hidden;"><div style="display:inline;" class="yAxis-Labels">'+intYLabels+'</div><div class="markerLine"></div></div>');
			}
		...
		function buildXAxis (objSettings, strID) {
			...
			for (intLabels = 0; intLabels < objSettings.chartData.DATA.length; intLabels++) {
				$(strID + " > .ddchart-x-axis-wrapper > .ddchart-x-axis-wrapper-sub").append('<div class="ddchart-x-axis" style="left:'+intBarWidth*intLabels+'%; width:'+intBarWidth/2+'%;"><div class="x-axis-text" title="'+objSettings.chartData.DATA[intLabels][objSettings.chartData.COLUMNS.indexOf('X_BAR_LABEL')]+'">'+objSettings.chartData.DATA[intLabels][objSettings.chartData.COLUMNS.indexOf('X_BAR_LABEL')]+'</div></div>'); // /*style="display:inline"*/
			}
		...
		function buildChart (objSettings, strID, intYMax) {
			...
			IntBarWidth = 100/objSettings.chartData.DATA.length/2;

			// alert(objSettings.chartData.DATA.length);
			// alert(intMargin);
			// alert(IntBarWidth);
			...
			for (intBars = 0; intBars < objSettings.chartData.DATA.length; intBars++) {
				...
				if (objSettings.chartData.COLUMNS.indexOf('URL_LINK') >= 0) {
					strURLLink = 'urlLink="'+objSettings.chartData.DATA[intBars][objSettings.chartData.COLUMNS.indexOf('URL_LINK')]+'"';
				}

				// console.log((objSettings.chartData.DATA[intBars][objSettings.chartData.COLUMNS.indexOf('X_BAR_VALUE')]/intYMax)*100);
				...
				if(getCurrentBranch()!=""){
					if(bcrumbCounter <= 1) {
						objSettings.horizontalLabel = true;
					}
				}			
				else {
					if(bcrumbCounter <= 2) {
				...
				if(objSettings.horizontalLabel) {
					$(strID + " > .ddchart-chart-final > .ddchart-chart-wrapper-sub").append('<div class="'+objSettings.chartBarClass+' ddchart-chart" '+strToolTipTitle+strDrillLink+strURLLink+' barlabel="'+objSettings.chartData.DATA[intBars][objSettings.chartData.COLUMNS.indexOf('X_BAR_LABEL')]+'" style="background-color:'+getRandomColor()+';left:'+((intBarWidth*intBars)+(intMargin/2))+'%; height:'+((objSettings.chartData.DATA[intBars][objSettings.chartData.COLUMNS.indexOf('X_BAR_VALUE')]/intYMax)*100)+'%; width:'+(((intBarWidth-intMargin)/2)+1)+'%;'+strBGColor+'" chartParentID="'+strID+'"><div class="barLabelsHori">'+objSettings.chartData.DATA[intBars][objSettings.chartData.COLUMNS.indexOf('X_BAR_LABEL')]+' <strong>'+objSettings.chartData.DATA[intBars][objSettings.chartData.COLUMNS.indexOf('X_BAR_VALUE')]+'</strong></div></div>');
					
					if(intBars >= objSettings.chartData.DATA.length-1) {
						objSettings.horizontalLabel = false;
					}
				}
				else {
					$(strID + " > .ddchart-chart-final > .ddchart-chart-wrapper-sub").append('<div class="'+objSettings.chartBarClass+' ddchart-chart" '+strToolTipTitle+strDrillLink+strURLLink+' barlabel="'+objSettings.chartData.DATA[intBars][objSettings.chartData.COLUMNS.indexOf('X_BAR_LABEL')]+'" style="background-color:#81aafd;left:'+((intBarWidth*intBars)+(intMargin/2))+'%; height:'+((objSettings.chartData.DATA[intBars][objSettings.chartData.COLUMNS.indexOf('X_BAR_VALUE')]/intYMax)*100)+'%; width:'+(((intBarWidth-intMargin)/2)+1)+'%;'+strBGColor+'position:absolute;" chartParentID="'+strID+'"><div class="barLabelsNew">'+objSettings.chartData.DATA[intBars][objSettings.chartData.COLUMNS.indexOf('X_BAR_LABEL')]+' <strong>'+objSettings.chartData.DATA[intBars][objSettings.chartData.COLUMNS.indexOf('X_BAR_VALUE')]+'</strong></div></div>');
			...
			if (objSettings.chartData.COLUMNS.indexOf('DRILL_DATA') > -1) 
				$(strID + " > .ddchart-chart-final > .ddchart-chart-wrapper-sub > .ddchart-chart").click(function() {
					if($(this).attr('barlabel')!='Total Farmers')
					{
				...
				if (objSettings.chartData.COLUMNS.indexOf('DRILL_LINK') > -1) 
					...
					if (objSettings.chartData.COLUMNS.indexOf('URL_LINK') > -1) 
							$(strID + " > .ddchart-chart-final > .ddchart-chart-wrapper-sub > .ddchart-chart").click(function() {
								window.open('http://'+$(this).attr('urlLink'),'_blank');	
							});
		...
		function setupChart (objSettings, strID) {
			...
			$(".ddchart-x-axis-wrapper-sub").remove();
		}
